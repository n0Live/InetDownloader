...
{* Автор: noLive (egor.krasnopolin@gmail.com)       *}
{* 09/07/2009                                       *}
{*                                                  *}
...
{* +--Connection--                                  *}
{* Password -  пароль,  для  аутентификации  на     *}
{*   сервере.                                       *}
...
{* +------------------                              *}
{* +--FtpParameters--                               *}
{* FtpThroghHttpProxy - работа с FTP через HTTP     *}
{*   прокси-сервер                                  *}
{* Passive - использование пассивного режима работы *}
{*   FTP                                            *}
{* +-----------------                               *}
...
{* Name - имя компонента.                           *}
{* +--Proxy--                                       *}
{* Bypass - список адресов, для которых не требуется*}
{*   использовать прокси-сервер. Адреса разделяются *}
{*   символом ";", разрешается использование масок  *}
{*   и макроса <local>.                             *}
{* Password - пароль к прокси-серверу.              *}
{* ProxyServer - адрес  прокси-сервера  в  формате: *}
{*   url:port или http=url:port ftp=url:port...     *}
{* UserName - логин к прокси-серверу.               *}
{* +---------                                       *}
{* +--PostQuery--                                   *}
{* Headers - заголовки POST/Custom-запроса.         *}
...
{* Query - строка POST/Custom-запроса.              *}
{* +-------------                                   *}
{* +--RepeatOnError--                               *}
...
type
 ...
 //====Сортируем свойства по подгруппам====
  ...
  private
    ...
    FUserName: string;                      //  Имя пользователя для Autentification: Basic
    FPassword: string;                      //  Пароль для Autentification: Basic
  ...
  //Свойства Файла
  TFileProperties = class(TPersistent)
  private
  ...
  TFtpParameters = class(TPersistent)
  private
    FFtpThroghHttpProxy: boolean;           //  Работа с FTP через HTTP прокси-сервер
    FPassive: boolean;                      //  Использовать пассивный режим работы FTP
   published
    property FtpThroghHttpProxy: boolean read FFtpThroghHttpProxy write FFtpThroghHttpProxy
      default False;
    property Passive: boolean read FPassive write FPassive
      default False;
  end;
  //Параметры загрузки
  TLoadParameters = class(TPersistent)
  private
  ...
  private
    FProxyServer: string;                   //  Адрес прокси-сервера
    FUserName: string;                      //  Имя пользователя для прокси
    FPassword: string;                      //  Пароль для прокси
    FBypass: string;                        //  Адреса, для которых не нужно использовать прокси-сервер
  published
  ...
  TPostQuery = class(TPersistent)
  private
    FPostType: TPostType;                   //  Способ отправки POST/Custom запроса
    FQuery: string;                         //  Строка POST/Custom запроса
    FHeaders: TStringList;                  //  POST/Custom-запрос в виде заголовков
  ...
  published
    ...
    property Headers: TStringList read FHeaders write SetPostHeaders;
  end;
  //Параметры повтора при ошибках
  TRepeatOnError = class(TPersistent)
  private
  ...
  //Заголовки запроса
  TRequestHeaders = class(TPersistent)
  private
...
//===== TInetDownloader =========================
  ...
  private
    ...
    FConnection: TConnection;                   //+ Параметры соединения
    FFileProperties: TFileProperties;           //+ Свойства Файла
    FFtpParameters: TFtpParameters;             //+ Параметры соединения с FTP
    FInternetAccessType: TInternetAccessType;   //  Тип доступа
    FLoadParameters: TLoadParameters;           //+ Параметры загрузки
    FProxy: TProxy;                             //+ Настроки прокси-сервера
    FPostQuery: TPostQuery;                     //+ Параметры отправки POST-запроса
    FRepeatOnError: TRepeatOnError;             //+ Параметры повтора при ошибках
    FReportLevel: TMsgTypes;                    //+ Уровень выводимой информации
    FRequestHeaders: TRequestHeaders;           //+ Заголовки запроса
  ...
  public
    ...
    property ThreadsList: TThreadList read FThreadsList;//Список запущенных потоков
  published
    //  == Свойства ==
    ...
    property FileProperties: TFileProperties read FFileProperties write FFileProperties;
    property FtpParameters: TFtpParameters read FFtpParameters write FFtpParameters;
    property InternetAccessType: TInternetAccessType read FInternetAccessType write FInternetAccessType;
    property LoadParameters: TLoadParameters read FLoadParameters write FLoadParameters;
    property Proxy: TProxy read FProxy write FProxy;
    property PostQuery: TPostQuery read FPostQuery write FPostQuery;
    property RepeatOnError: TRepeatOnError read FRepeatOnError write FRepeatOnError;
    property ReportLevel: TMsgTypes read FReportLevel write FReportLevel;
    property RequestHeaders: TRequestHeaders read FRequestHeaders write FRequestHeaders;
...
begin
  ...
  //Создаем объекты - группы свойств
  FConnection := TConnection.Create;
  FFileProperties := TFileProperties.Create;
  FFtpParameters := TFtpParameters.Create;
  FLoadParameters := TLoadParameters.Create;
  FProxy := TProxy.Create;
  FPostQuery := TPostQuery.Create;
  FRepeatOnError := TRepeatOnError.Create;
  FRequestHeaders := TRequestHeaders.Create;
  //Заполняем параметры дефолтными значениями
  FRequestHeaders.FUserAgent := 'Mozilla/4.0 (compatible; InetDownloader)';
  FRequestHeaders.FAcceptTypes := '*/*';
  FReportLevel := [mOk, mError, mInfo, mResult, mMessage, mProgress, mDataSize, mBytesReaded, mSpeed];
  FLoadParameters.FLoadFrom := 0;
  FLoadParameters.FLoadCount := 0;
  FFileProperties.FAutoFileName := False;
  FShowErrorDlg := True;
  FRepeatOnError.FRepeatOnError := False;
  FFtpParameters.FFtpThroghHttpProxy := False;
  FFtpParameters.FPassive := False;
  FRepeatOnError.FRepeatCount := 3;
  FRepeatOnError.FRepeatTimeout := 5000;
  FLoadParameters.FSpeedLimit := 0;
  FBufferSize := 1024;
  FRequestMethod := 'GET';
  FPostQuery.FPostType := ptNone;
  FInternetAccessType := atPreconfig;
  FFileProperties.FWriteMethod := wmRewrite;
  ...
  FPostQuery.FHeaders := TStringList.Create;
  FPostQuery.FHeaders.Add('Content-type: multipart/form-data, boundary=AaB03x542d');
...
begin
  //Освобождаем объекты - группы свойств
  FreeAndNil(FConnection);
  FreeAndNil(FFileProperties);
  FreeAndNil(FFtpParameters);
  FreeAndNil(FLoadParameters);
  FreeAndNil(FProxy);
  FreeAndNil(FPostQuery);
  FreeAndNil(FRepeatOnError);
...
procedure TConnection.SetUrl(const Value: string);
...
//Ввод PostHeaders
procedure TPostQuery.SetPostHeaders(const Value: TStringList);
{Value - проверяемое значение}
begin
...
begin
  ...
  if (FFileProperties.FFileName = '') and not FFileProperties.FAutoFileName then
    begin
...
//====== Заполняем параметры скачивания =============
  with Parameters do begin
    ...
    AcceptTypes := FRequestHeaders.FAcceptTypes;
    UserAgent := FRequestHeaders.FUserAgent;
    Referer := FRequestHeaders.FReferer;
    Url := FConnection.FUrl;
    AuthUserName := FConnection.FUserName;
    AuthPassword := FConnection.FPassword;
    FileName := FFileProperties.FFileName;
    PostType := FPostQuery.FPostType;
    PostHeaders := FPostQuery.FHeaders;
    PostQuery := FPostQuery.FQuery;
    RequestMethod := FRequestMethod;
    ProxyServer := FProxy.FProxyServer;
    ProxyBypass := FProxy.FBypass;
    ProxyUserName := FProxy.FUserName;
    ProxyPassword := FProxy.FPassword;
    ReportLevel := FReportLevel;
    LoadFrom := FLoadParameters.FLoadFrom;
    LoadCount := FLoadParameters.FLoadCount;
    RepeatOnError := FRepeatOnError.FRepeatOnError;
    RepeatCount := FRepeatOnError.FRepeatCount;
    RepeatTimeout := FRepeatOnError.FRepeatTimeout;
    AutoFileName  := FFileProperties.FAutoFileName;
    TypeOfData := FTypeOfData;
    WriteMethod := FFileProperties.FWriteMethod;
    BufferSize := FBufferSize;
    SpeedLimit := FLoadParameters.FSpeedLimit;

    UploadingString := FUploadingString;
    UploadingStream := FUploadingStream;
    Downloading := FDownloading;
    FtpThroughHttpProxy := FFtpParameters.FFtpThroghHttpProxy;
    FtpPassive := FFtpParameters.FPassive;
  end;
//===================================================
...
begin
  hSession := InternetOpen(PChar(FTParameters.UserAgent),  //User Agent
                           ...
                           PChar(FTParameters.ProxyServer),//Адрес прокси сервера
